...
function handleMove(ws, data, roomManager, broadcast) {
  ...
  broadcast(room.id, {
    ...
    currentZone: result.user.currentZone
  })
...
function handleChatMessage(ws, data, roomManager, broadcast) {
  ...
  if (targetUserId) {
    ...
    // Enviar para o destinatário (implementação simplificada)
    broadcast(room.id, message)
  } else {
    // Mensagem para grupo de proximidade
...
function handleWebRTCSignaling(ws, message, roomManager, broadcast) {
  ...
  // Encaminhar sinal para o destinatário
  broadcast(room.id, {
    type,
    fromUserId: clientId,
    targetUserId,
